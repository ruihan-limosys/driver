  ...
  const getLocationUpdates = async () => {
    ...
    watchId.current = Geolocation.watchPosition(
      async (position) => {
          ...
          const filteredLoc = await GpsKalman.process(position?.coords?.latitude, position?.coords?.longitude, position?.coords?.altitude, position?.timestamp);
          


        setLocation(filteredLoc);
